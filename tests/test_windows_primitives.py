...
# The production modules are in the repository root, which is not on sys.path
# when this file is run directly -- the investigation job does exactly that.
...
class CredentialPrimitiveTests(unittest.TestCase):
    ...
    The portable tests pin the declarations and the logic around the calls;
    this is the only place the calls themselves actually execute.  Diagnostics
    are printed because whether the privacy check passes on a stock box is a
    fact to observe, not to assume.
    """
